...
insert into exhibition(title, sub_title,discription,author,category,place,url,on_off,thumbnail)
				values('김인표의 화려한 시각예술','시각이란 무엇인가?','마서현이 춤춘다. 시각을 포기할 것 인가?',
					'김인표','시각','휴스타 강의실','localhost:8080',true,'thumbnail domain');
					
insert into exhibition_period(exhibition_idx, `date`, start_time, end_time)
						values(3,date_add(now(), interval 10 day),now(),date_add(now(), interval 4 hour));
...
>>>>>>> branch 'master' of https://github.com/inhwanK/artfarm.git
